...
class DeactivatorTest extends WP_UnitTestCase {
	...
	public function test_clears_campaign_lifecycle_cron(): void {
		wp_schedule_event( time(), 'hourly', 'mission_campaign_lifecycle' );
		$this->assertNotFalse( wp_next_scheduled( 'mission_campaign_lifecycle' ) );

		Deactivator::deactivate();

		$this->assertFalse( wp_next_scheduled( 'mission_campaign_lifecycle' ) );
	}
