...
class AdminModuleTest extends WP_UnitTestCase {
	...
	public function test_submenu_pages_are_registered(): void {
		...
		$this->assertContains( AdminModule::SUBSCRIPTIONS_SLUG, $submenu_slugs, 'Subscriptions submenu missing.' );
		$this->assertContains( AdminModule::SETTINGS_SLUG, $submenu_slugs, 'Settings submenu missing.' );
		$this->assertContains( AdminModule::TOOLS_SLUG, $submenu_slugs, 'Tools submenu missing.' );
		$this->assertCount( 7, $submenu[ AdminModule::MENU_SLUG ] );
	}
	...
	/**
	 * Test that the tools page renders expected content.
	 */
	public function test_tools_page_renders(): void {
		$page = new ToolsPage();
		...
		$this->assertStringContainsString( 'id="mission-admin"', $output );
		$this->assertStringContainsString( 'data-page="tools"', $output );
	}
}
