...
// Attributes.
$heading         = $attributes['heading'] ?? __( 'Recent Donors', 'mission-donation-platform' );
...
>
	...
	<?php if ( empty( $donors ) ) : ?>
		<div class="mission-donor-empty">
			...
			<p class="mission-donor-empty__title"><?php esc_html_e( 'Nothing here yet!', 'mission-donation-platform' ); ?></p>
			<p class="mission-donor-empty__subtitle"><?php esc_html_e( 'Your name could be the first one on this list.', 'mission-donation-platform' ); ?></p>
	...
	<?php else : ?>
		<ul class="mission-donor-list">
			...
			foreach ( $donors as $donor ) :
				...
				$time_ago = sprintf( __( '%s ago', 'mission-donation-platform' ), human_time_diff( strtotime( $donor['date_created'] ) ) );
				?>
 ...
 */
echo wp_kses( apply_filters( 'mission_recent_donors_output', $output, $campaign, $attributes ), \MissionDP\Helpers\Kses::block_allowed_html() );
